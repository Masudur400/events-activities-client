...
import { Search, RefreshCw, Eye, Trash2, Edit2, Calendar, Clock, Users, Tag, User, Mail, Phone, MapPin, Shield, Check, Star } from 'lucide-react'
...
import UpdateEventByAdminModal from './UpdateEventByAdminModal';
import { useDeleteEventByAdmin } from '@/hooks/event/useDeleteEventByAdmin';
...
const AllEventsTable: React.FC<MyEventsTableProps> = ({ eventTypes }) => {
...
useEffect(() => {
    ...
    const { mutate: deleteEvent, isLoading: deleting } = useDeleteEventByAdmin() as any
                        ...
                        {isLoading ? (
                            ...
                            events.map((event) => (
                                <tr key={event._id} className="hover:bg-gray-50/80 dark:hover:bg-gray-900 transition-colors group">
                                    ...
                                    <td className="p-4">
                                        {/* Action Cell with internal wrapper */}
                                        <div className="flex items-center justify-center gap-2">
                                            ...
                                            <button onClick={() => openUpdateModal(event)} className="p-2 cursor-pointer text-white bg-amber-600 hover:bg-amber-700 rounded-lg transition-all" title="Edit">
                                                <Edit2 className="w-4 h-4" />
                                            </button>
                                            <button onClick={() => { setDeleteId(event._id); setAlertOpen(true) }} className="p-2 cursor-pointer text-white bg-red-500 hover:bg-red-600 rounded-lg transition-all" title="Delete">
                            ...
                            <div className="p-6 grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
                                ...
                                <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-700 p-4 rounded-xl shadow-sm hover:shadow-md transition">
                                    ...
                                    <div className=''>
                                        ...
                                        <p className="font-semibold dark:text-white flex gap-2 items-center">{selectedEvent?.avgRating || 0} <Star className='w-4 h-4 text-yellow-500'></Star> ({selectedEvent.totalReviews} reviews)</p>
                                    </div>
                                </div>
