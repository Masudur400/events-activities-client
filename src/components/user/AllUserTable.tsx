...
import { useAllUsers, User } from '@/hooks/user/useAllUsers'
...
const AllUsersTable = () => {
    ...
    const { data, isLoading, refetch, isFetching } = useAllUsers({ searchTerm, page, limit: 10 })
    ...
    return (
        <div className="p-6 rounded-xl shadow-sm border border-gray-100 dark:border-yellow-700 w-full">
            ...
            <div className="mb-4 flex gap-2 items-center">
                <p className='font-medium'>Total Users ({totalUsers})</p>
                <input
                    type="text"
                            ...
                            users.map((user) => (
                                <tr key={user._id} className="hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors">
                                    ...
                                    <td className="p-4 text-center flex justify-center gap-2">
                                        <button onClick={() => openModal(user)} title="View" className="p-2 bg-blue-500 text-white rounded-lg"><Eye className="w-4 h-4" /></button>
                                        <button onClick={() => openUpdateModal(user)} title="Edit" className="p-2 bg-amber-600 text-white rounded-lg"><Edit2 className="w-4 h-4" /></button>
                                    </td>
                                </tr>
                            ))
                        )}
